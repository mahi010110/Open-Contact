/* ============================================================
   OpenContact — moteur · client de l’ordinateur (canal local)
   La PWA du même ordinateur découvre l’ordinateur sur 127.0.0.1,
...
/* la découverte : le premier port qui répond en Ordinateur */
export async function probeOrdinateur(){
  for (const port of ORDINATEUR_PORTS){
    try {
      const ctl = new AbortController();
      const t = setTimeout(() => ctl.abort(), 900);
      const r = await fetch(`http://127.0.0.1:${port}/oc-natif`,
...
export async function pairOrdinateur(base, code, saltB64, device, ring){
  ...
  if (!rep || !rep.k || !rep.ordinateur) throw new Error('format');
  return rep;   /* { ordinateur: {id, name, pub, role}, k } */
}

/* la conversation courante, sous la clé de canal */
export async function ordinateurCall(base, kB64, msg){
