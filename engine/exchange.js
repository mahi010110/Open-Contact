/* ============================================================
   OpenContact — moteur · échange (.oc)
   Lecture des fichiers reçus (OC2 chiffré, OC1 hérité, JSON),
   vue communautaire SANS données privées, enveloppes d'export.
   C'est ici — et seulement ici — que vit le format .oc.
   ============================================================ */
import { decryptOC2, unsealOC1 } from './crypto.js';
...
/* enveloppes d'export : « share » ne contient jamais le privé (communityView),
   « full » est la sauvegarde personnelle complète */
export function sharePayload(list){
  return { v: 4, app: APP_VERSION, kind: 'share', companies: list.map(communityView) };
}
export function fullPayload(companies, profile){
  return { v: 4, app: APP_VERSION, kind: 'full', profile, companies };
}
