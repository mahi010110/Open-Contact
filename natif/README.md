...
> **Chantier en attente, rien n'est livré.** Le code est écrit et testé,
> mais aucun paquet n'est maintenu — la release d'essai v0.1.0 (juillet
> 2026) ne doit pas être installée — et les capacités de cette surface
> sont masquées dans l'app web (`ui/perimetre.js`). Voir
> [`docs/surfaces.md`](../docs/surfaces.md) pour l'état réel et l'ordre
> des chantiers.
>
> `natif/` est le dossier ; à l'écran les surfaces s'appellent
...
```
node natif/preparer.mjs        # copie engine/ + tokens dans app/moteur/
cargo test -p oc-coeur             # la garde (18 tests, vecteurs croisés)
cargo build -p oc-natif        # la coquille (webkit2gtk + gtk3 requis sous Linux)
```

Le bundle passe par la CLI Tauri depuis `natif/src-tauri` (elle lance
...
sous des noms STABLES (ceux que la feuille « Ajouter l’ordinateur » de la
PWA sait choisir — `engine/distribution.js`), chaque paquet installé/lancé
et sondé (`GET /oc-natif`) avant publication.

## Le canal local (résumé du protocole)

`GET /oc-natif` — découverte **anonyme** (répond à toute origine) :
...
`oc-natif --mcp` sert le protocole MCP sur **stdio** (SDK officiel
...
Ordinateur), relue à CHAQUE appel. Deux outils, pas un de plus :
...
`tests/e2e/e2e-ordinateur-*.mjs` utilisent ces crochets sous xvfb.
