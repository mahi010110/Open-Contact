// OpenContact Ordinateur — la coquille (D17) : vie du processus,
...
fn main() {
    /* Mode serveur MCP local (P8-2) : c'est le client IA compatible qui
       lance `oc-natif --mcp` — pas de fenêtre, pas de canal, stdout
    ...
    builder
        ...
        .setup(move |app| {
            ...
            if let Ok(code) = std::env::var("OC_APPAIRAGE_AUTO") {
                eprintln!("ordinateur : APPAIRAGE DE TEST ACTIF — ne jamais utiliser en production");
                let mut sel = [0u8; 16];
            ...
            // l’ordinateur de vivre : la fenêtre reste le poste de repli
            if !integration_test {
                let tray = (|| -> tauri::Result<()> {
                    ...
                    TrayIconBuilder::with_id("principal")
                        ...
                        .tooltip("OpenContact Ordinateur")
                        .menu(&menu)
                ...
                if let Err(e) = tray {
                    eprintln!("ordinateur : zone de notification indisponible ({e})");
                }
            }
            println!("ordinateur : prêt");
            Ok(())
        })
        .on_window_event(|w, e| {
        ...
        .expect("démarrage de l’ordinateur");
}
