//! La détection des réponses (D8) : l’ordinateur lit la boîte avec le
...
pub fn detecter(p: &Arc<Partage>) {
    ...
    let vues = if let Ok(test) = std::env::var("OC_IMAP_TEST") {
        ...
        let Ok(mut sess) = client.login(&r.utilisateur, &mdp).map_err(|e| e.0) else {
            eprintln!("ordinateur : lecture refusée — vérifie le mot de passe d'application");
            return;
        };
        let v = chercher(&mut sess, &candidats);
        let _ = sess.logout();
    ...
    for cid in vues {
        ...
        println!("ordinateur : réponse détectée — relances arrêtées ({cid})");
    }
    em.sauver(&p.coffre);
}
