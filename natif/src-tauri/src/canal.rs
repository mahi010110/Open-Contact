...
pub fn demarrer(p: Arc<Partage>, app: tauri::AppHandle) {
    std::thread::spawn(move || {
        for port in PORTS {
            match Server::http(("127.0.0.1", port)) {
                Ok(srv) => {
                    ...
                    println!("ordinateur : canal local prêt ({port})");
                    boucle(srv, p, app);
                    return;
                }
                Err(_) => continue,
        ...
        eprintln!("ordinateur : canal local indisponible (ports occupés)");
    });
}
    ...
    match (methode, url) {
        ...
        (Method::Post, "/appairage") => {
            ...
            let reponse = serde_json::json!({
                "ordinateur": { "id": p.id, "name": p.nom, "pub": p.pub_b64url, "role": "ordinateur" },
                "k": b64(&k)
            });
            let kc = a.kc;
            drop(ap);
...
fn repondre_boite(
    ...
    match msg["t"].as_str() {
        ...
        /* L'anneau évolue après l'appairage (l’ordinateur lui-même,
        ...
        Some("mission") => {
            ...
            em.sauver(&p.coffre);
            println!("ordinateur : mission reçue {}", ms.mid);
        ...
        Some("revoquer") => {
            ...
            em.sauver(&p.coffre);
            println!("ordinateur : mission révoquée {mid}");
            serde_json::json!({ "t": "ok" })
        }
