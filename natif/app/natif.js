/* Ordinateur — le cerveau (webview). Il exécute les MÊMES modules
...
async function majEtat(){
  try {
    const e = await invoke('etat_ordinateur');
    ...
    q('#cgAssoc').textContent = 'Coquille injoignable — lance l’ordinateur, pas la page seule.';
  }
}
await majEtat();
...
q('#cgPair').addEventListener('click', async () => {
  try {
    ...
    q('#cgPairHint').textContent =
      'Recopie ce code dans OpenContact : Moi → Mes appareils → « Ajouter l’ordinateur ». Il expire dans 2 minutes.';
  } catch (e) {}
});
...
try {
  ...
  q('#cgMoteur').textContent = 'moteur partagé absent — lance `node natif/preparer.mjs`.';
}
