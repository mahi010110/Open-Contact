...
| V7 | CI reproductible : `.github/workflows/ci.yml` (auto-tests PWA, `cargo test --locked` + build release, suite E2E complète sous xvfb avec captures en artefacts) + `paquets.yml` (bundles non signés Linux/Windows/macOS à la demande — la signature reste au mainteneur) | Chaque poussée vers `main` et chaque PR rejouent ce que le développement joue localement | V5 | **terminée** | Workflows observés verts sur GitHub après poussée |
