...
| Multi-sélection (choisir quoi partager) | `.pk` avec icônes checkbox/checkbox-on — **plus utilisé pour supprimer**. L'emphase suit le DÉFAUT de la liste : si l'on part de **rien coché** (Prospecter), l'aplat de sélection marque le choix ; si l'on part de **tout coché** (Donner, partage en groupe, « → qui », une proposition d'IA à trier), la liste porte `pk-inverse` — la ligne retenue redevient calme et c'est l'**écart** qui se marque (dither + encre éteinte). Un aplat sur toutes les lignes n'est pas un signal |
