/* E2E D5 : rédaction IA « via ton ordinateur » contre le VRAI binaire.
   ...
   Ordinateur. Annuler libère vraiment (pas de verrou « occupé »).
   Ordinateur éteint = message court honnête. Mobile 390×844 sombre
...
/* ---------- le vrai Ordinateur — lancé APRÈS le déverrouillage de la
   PWA (le code d'appairage OC_APPAIRAGE_AUTO expire en 2 min) ---------- */
const xdg = mkdtempSync(path.join(os.tmpdir(), 'oc-natif-ia-'));
...
const errors = [];
page.on('console', m => {
  /* ordinateur volontairement éteint (fin du scénario) et 401 injecté :
...
if (fuiteCode === 0) fail('LA CLÉ EST ÉCRITE SUR LE DISQUE DU ORDINATEUR');
console.log('OpenAI : Bearer reçu, modèle choisi transmis, clé jamais écrite chez l’ordinateur ✓');
...
const annulation = await page.evaluate(async () => {
  ...
  const { base } = await probeOrdinateur();
  const j1 = 'ia-annultest1';
  const d1 = await ordinateurCall(base, assoc.k, { t: 'ia-demarrer', jid: j1,
    provider: 'ollama', key: '', model: 'llama9-test:8b',
    prompt: 'ATTENDS — rédige lentement', system: '' });
  await ordinateurCall(base, assoc.k, { t: 'ia-annuler', jid: j1 });
  const e1 = await ordinateurCall(base, assoc.k, { t: 'ia-etat', jid: j1 });
  const d2 = await ordinateurCall(base, assoc.k, { t: 'ia-demarrer', jid: 'ia-annultest2',
...
console.log('Ordinateur éteint : refus court, rien de perdu ✓');
await page.waitForTimeout(300);
...
ollama.close();
openai.close();
console.log(process.exitCode ? 'E2E ordinateur-ia : ÉCHEC' : 'E2E ordinateur-ia : OK');
