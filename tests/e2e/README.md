...
| `e2e-perte.mjs` | **« J'ai perdu mon téléphone »** (`docs/roadmap.md` §1.4) : la copie sort par « Télécharger » et se relit sur un appareil qui n'a JAMAIS vu ces données — clair/chiffré × sans coffre/avec coffre. Le suivi **privé** (notes, statut, historique, prochaine action) et la déclaration « j'y suis passé » survivent au **rechargement**, et sous coffre la copie restaurée s'écrit scellée. Épingle aussi ce que la copie n'emporte **pas** (les PDF), et rejoue la montée depuis `oc_data_v2`, `ais_stage_targets_v1` et un `.oc` du format d'origine |
