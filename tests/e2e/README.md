...
| Scénario | Ce qu'il prouve |
|---|---|
| `e2e-liaison.mjs` | **La liaison réelle**, pas simulée : deux vrais navigateurs et un relais Nostr local, la chaîne entière jouée — bibliothèque → WebSocket → découverte → WebRTC → échange. Sync « Mes appareils » dans les deux sens, partage en groupe avec aperçu avant fusion, rendez-vous QR — et celui-ci attend désormais **30 secondes** avant que l'autre rejoigne, le temps qu'une vraie personne sorte son téléphone (mesuré aussi à 0, 10, 25, 45 et 75 s ; la bibliothèque fait expirer les offres). Et les pannes **dites** : aucun relais joignable → l'écran le dit, plus jamais « en liaison » dans le vide. **Le NAT d'opérateur est reproduit** — la panne que les gens rencontrent vraiment, et que rien ne jouait : on retire au receveur tout candidat ICE, les « trickle » ET ceux embarqués dans le SDP distant (sans ce second, deux pages de la même machine se relient par la boucle locale et le transfert RÉUSSIT — une première version l'a cru reproduit alors qu'elle ne prouvait rien), puis on exige que les deux écrans NOMMENT la panne et proposent le repli. Enfin la cause de l'échec : les 4 rappels `onJoinError` transmettent leur erreur, et tout rendu de `rtcfail` branche dessus — une seule exemption, nommée, qui se vérifie elle-même. Et **un aperçu fermé sans fusionner n'a pas consommé l'envoi** : l'empreinte se pose à la réception, si bien que le receveur qui regarde, hésite et referme se retrouvait piégé — le même envoi renvoyé était jeté en silence pendant que l'expéditeur lisait « Parti vers 1 camarade ✓ ». Les DEUX moitiés sont tenues, sinon « ne jamais oublier » passerait le contrôle en rouvrant la feuille à chaque re-clic |
...
| `e2e-etats.mjs` | **Les deux états que personne ne met en place — vide, et plein.** §5 le dit (« un contrôle ne garde que les ÉTATS qu'il met en place ») et toutes les autres gardes sèment quinze à trente pistes : les deux bouts de l'échelle n'étaient mesurés par personne. L'**app vide**, c'est le premier lancement — le seul écran que 100 % des utilisateurs voient : les quatre écrans doivent enseigner le produit, et chaque feuille doit s'ouvrir **ou dire pourquoi elle refuse** (§6, famille ① du toast). Avec son contre-contrôle : `openProspect` sort en silence, ce qui n'est correct QUE parce que son bouton est absent — les deux moitiés sont figées. L'**app pleine**, c'est deux camarades qui ont partagé : 1 000 pistes sur un processeur bridé ×8, plafond de rendu prouvé **avec** son « Voir les autres », sinon 940 pistes seraient injoignables. Deux pièges d'instrument sont gardés dans le fichier, tous deux payés en l'écrivant et tous deux dans le sens du faux vert : un toast lu sans être effacé rend celui de la feuille précédente, et semer sans recharger chronomètre une liste vide |
| `e2e-focus.mjs` | **L'anneau de focus se VOIT** (WCAG 2.2 SC 2.4.11). Rien ne le mesurait : le jeton portait un commentaire, « lisible partout », qui était une intention et pas un relevé. 150 contrôles photographiés non focalisés puis focalisés — **on ne lit pas le CSS**, on compte les pixels qui changent, seul relevé qui survive à un anneau rogné par un ancêtre ou peint sous un enfant opaque. Trois critères : l'**aire** (le plus facile des deux minimums du texte — un périmètre de 2 px, ou un trait de 4 px le long du plus petit côté ; ne mesurer que le premier accuse toute l'app, la faute déjà payée par la sonde des relais), le **contraste** ≥ 3:1 entre les deux états, et la **forme** — au moins trois bords sur quatre, coins exclus. Ce troisième critère vient du produit, pas de WCAG, et c'est lui qui a rattrapé le pire défaut du lot : un anneau à 103 % de l'aire exigée, donc au vert, que l'œil ne voyait pas du tout. Trois sondes remettent les trois défauts d'origine et doivent rougir |
...
| `e2e-colonnes.mjs` | **Dans une liste, une valeur a UN BORD — ou elle ne se balaie pas.** Généralisation d'un défaut signalé sur photo : dans « Échanger », le compte vivait dans la phrase du canal, donc sa position dépendait de la longueur du canal — quatre abscisses pour le compte, trois pour la date. NN/g (*The Anatomy of a List Entry*) : une liste se BALAIE, l'œil descend une colonne et compare des entrées, donc chaque information garde la même place d'une ligne à l'autre. **Le critère est mécanique et ne demande rien aux listes** : une valeur répétée doit avoir un bord stable — le gauche si elle est calée à gauche, le droit sinon. Exiger « tout est aligné » serait faux (une sous-ligne qui s'élide a un bord droit qui varie) ; une valeur dont les DEUX bords bougent n'est ancrée à rien. Mesure aussi l'écart entre une rangée et son contenu, parce que ce qu'on perçoit d'une liste est son **pas** — le défaut qui a suivi la correction (enveloppe à 256 px pour 44 px de contrôle) est passé sous les deux gardes du fil, qui mesuraient le contrôle. **Une colonne, c'est UNE cellule par rangée** : là où une classe paraît deux fois dans la même rangée, c'est une grille ou une grappe — elle a autant d'abscisses que de colonnes, par construction. Sans ce filtre le balayage accusait les puces d'« Affiner », dont la grille à deux colonnes est parfaitement tenue ; sur-accuser rend un rapport que personne ne corrige, faute déjà payée par la sonde des relais puis celle du focus. 19 surfaces × 6 tailles, 1 108 lignes de liste ; tout ce qui reste dehors est masqué par `ui/perimetre.js` (§0). **Une seule exemption, nommée à la classe ET à la liste** : les deux colonnes de données du fil, parce que leur alignement s'est révélé arithmétiquement impossible sous 390 px — réserver au compte les 6,25 rem que son vocabulaire demande (« rien de neuf », « 1480 complétées ») chasse le chevron sur un second étage, sur toutes les combinaisons de colonne, de date et de gouttière essayées. Le mécanisme qui alignait tout (`subgrid`) a cassé l'app sur un vrai téléphone : il n'était vérifié que sur Chromium. Deux sondes, qui ont servi tout de suite : une fonction ne se sérialise pas en argument de `evaluate`, les deux revenaient muettes et le fichier l'a dit au lieu de conclure |
