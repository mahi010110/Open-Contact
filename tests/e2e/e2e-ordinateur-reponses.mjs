/* E2E C5 : la détection des réponses par le VRAI Ordinateur (IMAP).
   Faux IMAP local (OC_IMAP_TEST) : après les envois, « nadia »
   répond → l’ordinateur arrête ses relances tout seul et la PWA
...
/* ---------- le vrai Ordinateur ---------- */
const xdg = mkdtempSync(path.join(os.tmpdir(), 'oc-natif-rep-'));
...
let journalC = '';
ordinateur.stdout.on('data', d => { journalC += d; });
...
/* l’ordinateur détecte la réponse et arrête la cible, seul */
await attendre(() => /réponse détectée/.test(journalC), 30000, 'détection IMAP');
console.log('réponse détectée par l’ordinateur ✓');
...
const rapBrut = await page.evaluate(async () => {
  const { loadOrdinateur } = await import('./ui/ordinateur.js');
  const { probeOrdinateur, ordinateurCall } = await import('./engine/ordinateur.js');
  const assoc = await loadOrdinateur();
  const found = await probeOrdinateur();
  return await ordinateurCall(found.base, assoc.k, { t: 'rapport' });
});
...
sink.close();
imapFaux.close();
console.log(process.exitCode ? 'E2E ordinateur-réponses : ÉCHEC' : 'E2E ordinateur-réponses : OK');
