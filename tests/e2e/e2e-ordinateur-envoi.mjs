/* E2E C3+C4 : la campagne confiée, exécutée par le VRAI Ordinateur.
   ...
   l'assistant (« Mon ordinateur envoie tout seul »), l’ordinateur
   ...
   Prérequis : `cargo build -p oc-natif` fait (tous.mjs le saute
...
/* ---------- le vrai Ordinateur, crochets de développement ---------- */
const xdg = mkdtempSync(path.join(os.tmpdir(), 'oc-natif-e2e-'));
...
function lancer(){
  ...
  ordinateur.stdout.on('data', d => { journalBin = (journalBin + d).slice(-4000); process.stdout.write('[ordinateur] ' + d); });
...
console.log('vrai Ordinateur lancé, appairage ouvert ✓');
...
/* appairage réel (le parcours UI est couvert par e2e-ordinateur.mjs) */
...
console.log('appairé au vrai Ordinateur ✓');
...
/* l’ordinateur envoie — pour de vrai, jusqu'au puits SMTP */
await attendre(() => messages.length >= 2, 30000, '2 envois SMTP de l’ordinateur');
...
lancer();
await attendre(async () => !!(await sonde()), 30000, 'Ordinateur relancé');
...
await page.waitForFunction(() => !!document.querySelector('#sbVer')?.textContent.trim(),
  null, { timeout: 10000 });
await page.evaluate(async () => {
  await (await import('./ui/campagnes.js')).reconcileOrdinateur();
});
...
server.close();
sink.close();
console.log(process.exitCode ? 'E2E ordinateur-envoi : ÉCHEC' : 'E2E ordinateur-envoi : OK');
