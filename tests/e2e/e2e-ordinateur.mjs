/* E2E C2 : l'appairage de l’ordinateur — la PWA (profil protégé,
   appareil principal) découvre un ordinateur sur l’ordinateur,
   prouve le code court, reçoit la clé de canal, l'inscrit dans
   l'anneau (rôle ordinateur), montre l'état « prêt », puis rompt.
   Le faux Ordinateur parle EXACTEMENT le protocole du vrai (mêmes
...
/* ---------- le faux Ordinateur (protocole du canal) ---------- */
...
const fauxSrv = http.createServer(async (req, res) => {
  ...
  try {
    if (req.method === 'GET' && req.url === '/oc-natif'){
      ...
      const rep = { ordinateur: { id: 'cgfaux', name: 'FauxOrdi', role: 'ordinateur',
...
/* attendus, pas des erreurs : le 403 du mauvais code (refus propre),
   les sondes du canal local quand l’ordinateur n'est pas là (404 puis
...
/* Mes appareils → Ajouter l’ordinateur. Il n'est pas encore installé
...
await page.route('https://api.github.com/**', r => r.fulfill({
  ...
  body: JSON.stringify({ tag_name: 'v0.1.0', assets: [
    { name: 'OpenContact-Ordinateur-windows-x64-setup.exe', browser_download_url: 'https://exemple.test/w.exe', size: 4000000 },
    { name: 'OpenContact-Ordinateur-linux-x64.deb', browser_download_url: 'https://exemple.test/l.deb', size: 21000000 },
    { name: 'OpenContact-Ordinateur-linux-x64.AppImage', browser_download_url: 'https://exemple.test/l.AppImage', size: 23000000 },
    { name: 'OpenContact-Ordinateur-macos-universel.dmg', browser_download_url: 'https://exemple.test/m.dmg', size: 11000000 }
  ] })
}));
await page.click('#devAddComp');
...
await page.screenshot({ path: SHOTS + '/70-ajouter-ordinateur.png' });
console.log('installation guidée : bon fichier Linux, honnêteté non-signé, replis ✓');

/* l’ordinateur vient d'être ouvert : « chercher » le trouve */
faux.visible = true;
...
if (etat.role !== 'ordinateur') fail('rôle dans l’anneau : ' + etat.role);
if (!faux.assoc || !faux.assoc.device || !faux.assoc.device.pub) fail('l’ordinateur n’a pas reçu l’identité');
if (!faux.assoc.ring || !faux.assoc.ring.main) fail('l’ordinateur n’a pas reçu l’anneau');
console.log('clé scellée, anneau (rôle ordinateur) et identités échangés ✓');

/* la feuille montre la ligne ordinateur, présence « prêt » */
...
await page.waitForTimeout(300);
await page.screenshot({ path: SHOTS + '/71-ordinateur-pret.png' });

/* rompre : l’ordinateur prévenu, la clé oubliée, l'anneau nettoyé */
...
const apres = await page.evaluate(async () => {
  ...
  const { getRing } = await import('./ui/synclive.js');
  const { deviceIn } = await import('./engine/ring.js');
  return { cle: await st.kvGet(st.ORDINATEUR_KEY), dans: !!deviceIn(getRing(), 'cgfaux') };
});
if (apres.cle) fail('la clé de canal devrait être oubliée');
if (apres.dans) fail('l’ordinateur devrait être sorti de l’anneau');
if (!faux.recus.some(m => m.t === 'dissocier')) fail('l’ordinateur n’a pas été prévenu de la rupture');
if (faux.assoc) fail('le faux Ordinateur se croit encore associé');
console.log('rupture propre : clé oubliée, anneau nettoyé, Ordinateur prévenu ✓');
...
server.close();
fauxSrv.close();
console.log(process.exitCode ? 'E2E ordinateur : ÉCHEC' : 'E2E ordinateur : OK');
