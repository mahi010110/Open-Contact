/* ============================================================
   OpenContact — interface · l’ordinateur (association, présence)
   « Ton ordinateur qui travaille pour toi » — jamais de jargon
   (D4). L'association se fait depuis l’ordinateur : la fenêtre du
   Ordinateur affiche un code court, on le recopie ici. La feuille
...
import { ORDINATEUR_KEY, kvGet, kvSet, kvDel } from '../engine/storage.js';
import { probeOrdinateur, pairOrdinateur, ordinateurCall, normCode } from '../engine/ordinateur.js';
...
import { deviceSelf, ensureKeys, getRing, amMain, ringDo, ringAddOrdinateur } from './synclive.js';
import { isProtected, requireCode, openProtectFlow } from './verrou.js';

const isDesktop = () => matchMedia('(min-width:901px)').matches;

/* pourquoi l’ordinateur — les mêmes trois raisons partout */
const whyHTML = () =>
  `<div class="lk-whys">
...
/* ---------- téléphone : l’ordinateur se prépare sur l’ordinateur ---------- */
export function openOrdinateurPhoneSheet(){
  const sh = openSheet({ title: 'L’ordinateur', icon: 'monitor' });
  sh.body.innerHTML =
    `${whyHTML()}
     <p class="hint" style="margin-top:10px">Il s’installe et s’associe <b>depuis ton ordinateur</b> :
        ouvre OpenContact là-bas, puis <b>Moi → Mes appareils → Ajouter l’ordinateur</b>.</p>
...
export async function loadOrdinateur(){
  try { return JSON.parse(await kvGet(ORDINATEUR_KEY) || 'null'); }
  catch (e) { return null; }   /* verrou : pas d'association lisible */
}

/* l'état vivant : prêt (répond sous la clé de canal) / éteint */
export async function ordinateurPresence(){
...
export function openAddOrdinateur(onDone){
  ...
  if (!isDesktop()){ openOrdinateurPhoneSheet(); return; }
  const sh = openSheet({ title: 'Ajouter l’ordinateur', icon: 'monitor', focus: '.x' });
  ...
  const gateAssoc = async () => {
    if (!isProtected()){
      ...
      sh.body.innerHTML =
        `<p class="hint" style="margin:0 0 12px">L’ordinateur est là ✓ — l’associer lui confie
    ...
    if (!(await amMain())){
      sh.body.innerHTML =
        `<p class="hint" style="margin:0 0 12px">Seul ton appareil principal peut associer l’ordinateur.</p>`;
  ...
  const stepProbe = async () => {
    ...
    if (!found){
      sh.body.innerHTML =
        `<p class="hint warn" style="margin:0 0 12px">Je ne trouve pas l’ordinateur ici.
  ...
  const stepFound = async found => {
    ...
    if (!found.info.appairage){
      sh.body.innerHTML =
        `<p class="hint" style="margin:0 0 12px"><b>${esc(found.info.nom || 'Ordinateur')}</b> est là ✓ —
  ...
  const stepCode = found => {
    sh.setTitle('Le code de l’ordinateur');
    sh.body.innerHTML =
    ...
    const go = async () => {
      ...
      try {
        ...
        await kvSet(ORDINATEUR_KEY, JSON.stringify({
          k: rep.k, id: rep.ordinateur.id, nom: rep.ordinateur.name || 'Ordinateur',
          pub: rep.ordinateur.pub, at: Date.now()
        }));
        await ringAddOrdinateur({ id: rep.ordinateur.id, name: rep.ordinateur.name || 'Ordinateur', pub: rep.ordinateur.pub });
        logJ('Ordinateur associé : ' + (rep.ordinateur.name || ''));
        sh.close(null, true);
        toast('Associé à « ' + (rep.ordinateur.name || 'ton ordinateur') + ' » ✓');
        bus.refresh();
        if (onDone) onDone();
      } catch (e) {
        q('#cgErr').textContent = e.message === 'code'
          ? 'Ce n’est pas ce code-là. Regarde la fenêtre de l’ordinateur.'
          : (e.message === 'ferme'
            ? 'Le code a expiré — refais « Afficher le code » dans l’ordinateur.'
            : 'Pas de réponse — l’ordinateur est toujours ouvert ?');
...
export function openOrdinateurSheet(assoc, onDone){
  const sh = openSheet({ title: assoc.nom || 'Ordinateur', icon: 'monitor' });
  sh.body.innerHTML =
  ...
  const q = s => sh.body.querySelector(s);
  let live = null;
  const majLive = () => ordinateurPresence().then(p => {
    live = p;
    const el = q('#cgLive');
    if (!el) return;
  ...
  q('#cgMcp').addEventListener('click', async () => {
    if (!live || live.state !== 'on'){
      toast('Ton ordinateur est éteint — ouvre l’ordinateur d’abord.');
      return;
    }
    const on = !!live.mcp;
    ...
    const apply = async actif => {
      ...
      try {
        const rep = await ordinateurCall(live.base, assoc.k, { t: 'mcp-regler', actif });
        if (!rep || rep.t !== 'ok') throw new Error('canal');
        logJ(actif ? 'Assistant IA autorisé sur l’ordinateur' : 'Assistant IA coupé sur l’ordinateur');
        sa.close(null, true);
        ...
        majLive();
      } catch (e) { toast('Pas de réponse — l’ordinateur est toujours ouvert ?'); }
    };
  ...
  q('#cgBreak').addEventListener('click', async () => {
    const ok = await confirmSheet({
      ...
      msg: `<b>${esc(assoc.nom || 'L’ordinateur')}</b> ne recevra plus de missions et oublie vos clés communes. Rien d’autre n’est effacé.`
    });
    if (!ok) return;
    if (!await requireCode('Ton code, pour rompre')) return;
    /* de bonne foi : prévenir l’ordinateur s'il répond, puis oublier */
    try {
      const found = await probeOrdinateur();
      if (found) await ordinateurCall(found.base, assoc.k, { t: 'dissocier' });
    } catch (e) {}
    await kvDel(ORDINATEUR_KEY);
    await ringDo('remove', assoc.id).catch(() => {});
    logJ('Ordinateur dissocié');
    sh.close(null, true);
